Feature: AMO003 Request Payment - Business Scenarios
  As APISYS
  ...
  Scenario: Idempotency: Request payment with same transaction_no should return same results
    Given the member has positive wallet balance in "<currency>"
    And I prepare a deduction amount of 10
    When APISYS requests payment with:
      | field             | value               |
  ...
  Scenario: Request payment supports up to 6 decimal places correctly
    Given the member has positive wallet balance in "<currency>"
    And I record the current wallet balance in "<currency>"
    When APISYS requests payment with:
      | field             | value               |
  ...
  Scenario: Request payment with positive amount violates business rule
    When APISYS requests payment with:
      | field             | value               |
      | transaction_no    | <transaction_no>    |
      | game_key          | <game_key_seamless> |
